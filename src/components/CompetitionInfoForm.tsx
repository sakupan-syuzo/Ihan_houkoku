...
export const CompetitionInfoForm: React.FC<CompetitionInfoFormProps> = ({
  ...
  const theme = {
    ...
    borderSub: isDarkMode ? 'border-slate-800' : 'border-gray-200',
    text: isDarkMode ? 'text-slate-200' : 'text-gray-900',
    textMuted: isDarkMode ? 'text-slate-400' : 'text-gray-600',
    textHighlight: isDarkMode ? 'text-amber-400' : 'text-amber-700',
  ...
  return (
    <section className={`${theme.cardBg} border ${theme.border} rounded-xl p-4 sm:p-5 shadow-lg space-y-4`}>
      <div className={`flex items-center justify-between border-b ${theme.borderSub} pb-2.5`}>
        <h2 className={`text-sm font-bold ${theme.text} flex items-center gap-2`}>
          <Trophy className={`w-4 h-4 ${theme.textHighlight}`} />
          大会・セッション基本情報
        </h2>
            ...
            <button
              ...
              className={`text-xs font-medium flex items-center gap-1 px-2.5 py-1 rounded-md transition-colors ${
                isDarkMode
                  ...
                  : 'text-emerald-700 hover:text-emerald-800 bg-emerald-50 border border-emerald-300'
              }`}
            >
          ...
          <button
            ...
            className={`text-xs font-medium flex items-center gap-1 px-2.5 py-1 rounded-md transition-colors ${
              isDarkMode
                ...
                : 'text-amber-700 hover:text-amber-800 bg-amber-50 border border-amber-300'
            }`}
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5">
        ...
        <div className="sm:col-span-2 space-y-1.5">
          <div className="flex items-center justify-between">
            ...
            {availableEvents.length > 0 && (
              <div className="flex items-center gap-1">
                <span className={`text-[11px] ${isDarkMode ? 'text-slate-500' : 'text-gray-500'}`}>ラウンド選択:</span>
                <select
                  value={formData.competitionName}
                  onChange={handleSelectRound}
                  className={`${theme.inputBg} ${theme.textHighlight} border ${theme.inputBorder} rounded px-2 py-0.5 text-xs focus:outline-none focus:border-red-500`}
                >
          ...
          <input
            ...
            className={`w-full ${theme.inputBg} border ${theme.inputBorder} rounded-lg px-3 py-2 text-sm ${theme.inputText} ${theme.placeholder} focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 transition-colors`}
          />
        </div>

        {/* ゼッケン No */}
        <div>
          <label className={`block text-xs font-semibold ${theme.textMuted} mb-1 flex items-center gap-1`}>
          ...
          <input
            ...
            placeholder="例: 1"
            className={`w-full ${theme.inputBg} border ${theme.inputBorder} rounded-lg px-3 py-2 text-sm font-bold text-red-400 ${theme.placeholder} focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 transition-colors`}
          />
        </div>

        {/* セッション選択 */}
        <div className="sm:col-span-2 lg:col-span-3">
          <label className={`block text-xs font-semibold ${theme.textMuted} mb-1.5 flex items-center gap-1`}>
            <Flag className={`w-3.5 h-3.5 ${theme.textHighlight}`} />
            セッション
          </label>
          <div className="flex flex-wrap gap-2 items-center">
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all ${
                    isSelected
                      ...
                      : `${theme.inputBg} ${theme.inputBorder} ${isDarkMode ? 'text-slate-300' : 'text-gray-700'} ${theme.hoverBg}`
                  }`}
                >
                  {label}
                </button>
            ...
            {isCustomSession && (
              <input
                ...
                className={`${theme.inputBg} border border-red-500 rounded-lg px-2.5 py-1 text-xs ${isDarkMode ? 'text-amber-300' : 'text-amber-700'} ${theme.placeholder} focus:outline-none flex-1 min-w-[150px]`}
              />
            )}
          </div>
        </div>
        ...
        <div>
          ...
          <input
            ...
            className={`w-full ${theme.inputBg} border ${theme.inputBorder} rounded-lg px-3 py-2 text-sm ${theme.inputText} focus:outline-none focus:border-red-500 transition-colors`}
          />
        </div>
        ...
        <div>
          <label className={`block text-xs font-semibold ${theme.textMuted} mb-1 flex items-center gap-1`}>
            <MapPin className={`w-3.5 h-3.5 ${theme.textMuted}`} />
            発生場所
          </label>
          <input
            type="text"
            ...
            className={`w-full ${theme.inputBg} border ${theme.inputBorder} rounded-lg px-3 py-2 text-sm ${theme.inputText} ${theme.placeholder} focus:outline-none focus:border-red-500 transition-colors`}
          />
        </div>
      </div>
    </section>
